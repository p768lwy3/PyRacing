...
class database(object):
    ...
    def horseupdate(self):
        ...
        for c in range(ord('A'), ord('Z') + 1):
            req = requests.get(url.format(chr(c)), headers = headers)
            if req.status_code == requests.codes.ok:
                ...
                for li in soup.find_all("li", {"class": "table_eng_text"}):
                    ...
                    try:
                        self.__updatehorseinfo__(horse_no)
                        self.__updatepastrecord__(horse_no)
                        time.sleep(2)
                    except:
    ...
    def __updatehorseinfo__(self, horse_no):
        ...
        if req.status_code == requests.codes.ok:
            ...
            try:
                seasonstakes = int(''.join([d for d in l[6] if d.isdigit()]))
            except:
                seasonstakes = 0
